...
import sys
sys.path.append('$/lib64/python2.5/site-packages')
...
class seqGen:
  ...
  def __init__(self, nRuns, T_start, T_end, lambdaFactor, maxCxSize=1, seqFile=None, bindingFile=None, saveRuns=[], mode="SA"):
    ...
    if len(sys.argv) > 1:
      ...
      self.job_server = pp.Serve(ncpus,ppservers=self.ppservers,secret='t6f3052K83')
    else:
      # Creates jobserver with automatically detected number of workers
      self.job_server = pp.Server(ppservers=self.ppservers,secret="t6f3052K83")
    #######################################################
    ...
    print os.path.abspath(os.path.join(os.getcwd(), os.path.pardir))+"/parameters.txt"
    self.concentrations1 = nupack.NUPACK(prefix=os.getcwd()+"/Concentrations/LowestErr",
                                paramFile=os.path.abspath(os.path.join(os.getcwd(), os.path.pardir))+"/parameters.txt",
                                silent=True, degenerate=False, material="dna", dangles="all", ordered=True, timeonly=False, mfe=True, quiet=True, sort=1)
    # initializing the NUPACK object in silent mode for the calculation of the concentrations in the second steady state
    self.concentrations2 = nupack.NUPACK(prefix=os.getcwd()+"/Concentrations/LowestError",
                                paramFile=os.path.abspath(os.path.join(os.getcwd(), os.path.pardir))+"/parameters.txt",
                                silent=True, degenerate=False, material="dna", dangles="all", ordered=True, timeonly=False, mfe=True, quiet=True, sort=1)
  ...
  def _mainloop(self, nRuns, T_start, T_end, lambdaFactor, screenOut=False):
    ...
    while T > T_end:
      ...
      if lambdaFactor>0:
        ...
        self.critonsChck = critonsCheck.CritonsCheck(run,nRuns,lambdaFactor,"/home/hroekel/DNAstranddesign_MM/TUPack_Upgrade_Zandra10_10/model/CritonsDataFile_Lambda15_seed1_Temp15000_1e6_","/home/hroekel/DNAstranddesign_MM/TUPack_Upgrade_Zandra10_10/model/CritonsInfoFile_Lambda15_seed1_Temp15000_1e6_",lc=3,strandsObject=self.Strands)
      ...
      if newError <= Error:
        ...
        if self.lowestError > Error:
          self.lowestError = Error
          self._saveRun(os.getcwd()+"/Concentrations/LowestErr",1)
          self._saveRun(os.getcwd()+"/Concentrations/LowestError",1)
  ...
  def _writeNpInput(self, fileName, maxCxSize, condition):
    ...
    if condition == 0:
      ...
      for strand in self.Strands:      
        cxFile.write(str(strand.Sequence)+"\n")
      cxFile.write(str(self.Strands[3].Sequence)+str(self.Strands[4].Sequence+"\n"))
      cxFile.write(str(self.Strands[4].Sequence)+str(self.Strands[5].Sequence+"\n"))
      cxFile.write(str(self.Strands[4].Sequence)+str(self.Strands[6].Sequence+"\n"))
      cxFile.write(str(3))
  ...
  def _writeConcentrationFile(self, ind, mult, conc, ener, fileName):
    ID = {1:'T1', 2:'T2', 3:'T3', 4: 'U', 5:'a', 6:'Y', 7:'inh', 8:'Ua', 9:'aY', 10:'a inh'}
...
prog = seqGen(1000000,15000,10,10,maxCxSize=2,
              seqFile='/home/hroekel/TUPack_Upgrade_Zandra10_10_1/model/sequences.txt',
              bindingFile='/home/hroekel/TUPack_Upgrade_Zandra10_10_1/model/binding.txt',
              saveRuns=[1,100000,200000,300000,400000,500000,600000,700000,800000,900000,1000000],
              mode='SA')
